...
14. risks[]: each risk MUST reflect a DOCUMENTED problem in the artifacts. Valid sources:
    ...
    FRESH ASKS ARE NOT RISKS: an open item being on the critical path does NOT by itself make it a risk. If we asked the customer for something <2 days ago and they haven't responded yet, that's normal — it's an open item with a recent asked_date, NOT a risk. Risks fire when there's evidence of stalling (item is 3+ days old), missed dates, or escalation — not from importance alone. Example: on Day 3 we asked Marcus to loop in Sam for DM access. Sam hasn't been directly contacted yet. That is NOT a high-severity risk — it's the natural flow. It becomes a risk only when the ask goes stale.
...
11. DRAFT COHERENCE (non-negotiable). Each Action is ONE cohesive message about ONE topic to ONE person. The following MUST all align with each other inside the SAME Action object:
    - "type" (internal action title)
    - "addresses.label" (the open item or risk being addressed)
    - "channel.to" (the recipient — for email: "First Last · email"; for Slack: handle/channel)
    - "channel.subject" (email subject line)
    - The greeting line of the "draft" body — for email, the first line is "Hi <FirstName>," where FirstName matches the first name in channel.to
    - The substance of the "draft" body — it discusses the SAME topic as the subject and addresses.label

    Before finalizing each draft, run this explicit self-check:
      (a) Re-read your own channel.to and pull out the recipient's first name.
      (b) Re-read the FIRST LINE of the body. Does the greeting name match the recipient first name? If not → the draft is broken, fix the greeting.
      (c) Re-read the subject and addresses.label. Does the body discuss that same topic? If not → the draft is broken, rewrite the body.

    NEVER blend context from two different drafts into one Action object. If you have two distinct actions to draft (e.g. a Marcus unblock email AND a Dani tour calendar invite), produce TWO separate Action objects in the drafts array — NOT one Action with Marcus's headers and Dani's body. Each Action is self-contained.

    BAD example (do NOT produce this):
      { type: "Loop Marcus in to unblock Sam Park access",
        channel: { via: "email", to: "Marcus Lee · marcus.lee@knix.com", subject: "One quick unblock — Sam Park intro" },
        draft: "Hi Dani, Loved that you asked for this at kickoff — happy to walk you through the review queue and drafting interface..." }
      → Recipient is Marcus but greeting says Dani. Subject is Sam-Park-intro but body is workflow-tour. This Action mixes two drafts. Forbidden.
12. MATCH THE CUSTOMER'S LITERAL ASK. When drafting a response to a customer-initiated open item (owner = "Strategist"), the draft body MUST stay on the topic the customer actually raised in their email/message. Use their words and frame. Do NOT pivot the topic to broader concerns from account.caresAbout, account.promised, or product-spec vocabulary the customer didn't bring up in THIS SPECIFIC ASK.
    Example: a customer asks for a "30-min workflow tour — show me the review queue and drafting interface." That is what the email should be about — what the queue looks like, the drafting UI, how their team interacts with it operationally. Do NOT inflate it into "how the agent handles sensitive customer questions" or "how the guardrails work" — even though sensitive handling is real product context, the customer asked for an operational tour, not a guardrails walkthrough. Save the broader topics for the alignment session they've mentioned wanting later (or when they raise those topics themselves).
    The subject line follows the same rule: match the customer's ask, not the broader product spec.`;
